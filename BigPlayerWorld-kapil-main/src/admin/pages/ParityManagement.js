...
const ParityManagement = () => {
  ...
  const sendResult = async (e) => {
    try {
      e.preventDefault();
      console.log("setNoSelected", noSelected);
      console.log({
        game_id: gameId,
        value: +noSelected,
      });
      const { data } = await UpdateParityResult(token, {
        game_id: gameId,
        value: noSelected,
      });
...
export default ParityManagement;
