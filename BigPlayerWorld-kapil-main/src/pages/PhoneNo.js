...
import { SendOTP, UserRegistration } from "../api";
...
function PhoneNo(props) {
  ...
  const [hasOtpErr, sethasOtpErr] = useState("");
  ...
  const validateInput = () => {
    ...
    if (!phone || !password || !otp) {
      console.log("!phone || !password || !otp", { phone, otp, password });
      if (!phone) setMobileErr("Mobile is required");
      if (!password) setPasswordErr("Password is required");
      if (!otp) sethasOtpErr("OTP is required");
      return false;
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (isValid) {
      ...
      if (isTokenFound) payload.token = isTokenFound;

      if (refCode) payload.share_referral_code = refCode;
      setIsLoading(true);
      setIsDisabled(true);
      UserRegistration(payload)
        ...
        .catch((e) => {
          ...
          if (e?.response?.data?.message) {
            if (
              e.response.data.message.mobile ==
  ...
  return (
    <div className="container gray">
      ...
      >
        ...
        <p style={{ fontWeight: "bolder", fontSize: "18px" }}>Register</p>
        <p></p>
      </div>
      <div className="auth_form">
        ...
        <div className="auth_form_otp">
          ...
          <div
            className="auth_form_options extra_width_sendotp"
            style={{ marginLeft: "10px" }}
          >
            <button
              disabled={isDisable}
              className={"auth_send_otp"}
              style={isDisable ? { color: "gray" } : null}
              onClick={(e) => {
                submitOTP(e);
              }}
            >
        ...
        <div className="auth_form_submit">
          <button
            ...
            style={isDisabled ? { color: "white" } : null}
