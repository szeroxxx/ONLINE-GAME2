...
function Parity(props) {
  ...
  const sendGamePlayDetails = async (payload) => {
    try {
      if (gameId) {
        ...
        payload.play_time="THREE_MINUTES"
  ...
  async function fetchData() {
    try {
      fetchMyOrder();
      if (!token) history("/");
      const { data } = await getThreeMinuteGameID(token);
  ...
  async function fetchMyOrder() {
    try {
      ...
      const { data } = await GetThreeMinuteOrders(token, payload);
      if (data?.result) {
  ...
  async function fetchOrders() {
    try {
      ...
      const { data: result } = await GetThreeMinuteOrders(token, orderpayload);
      if (data?.result) {
        setMyOrder(data.result);
      }
      if (result?.result) {
